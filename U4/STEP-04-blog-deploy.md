...
- `GYOZA WOOD 技術紀錄` 首頁
- 一篇標題是 `我如何用 AI coding agent 接手並交付 GYOZA WOOD 備料控制台` 的文章

**點進去看看**——它現在是一份**骨架**，每一節都只有灰色提示，還沒有內容。
那些提示就是你要填的地方。

## 2. 確認骨架填完了

你在 [`STEP-03-blog-write.md`](./STEP-03-blog-write.md) 應該已經寫得差不多了。
這一步只確認一件事——打開：
...
> 文章要有哪幾節、每節回答什麼——**全部寫在骨架檔案裡了**，這裡不重複列。
> 判斷標準只有一條：**每一段至少要有一個別人可以查證的東西**（檔名、數字、指令輸出、截圖）。
